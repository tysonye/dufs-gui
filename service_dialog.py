...
from design_system import DesignSystem, AnimatedButton
...
QGroupBox {{
    font-weight: 600;
    font-size: {DesignSystem.FONTS['size_lg']};
...
QPushButton {{
    ...
    padding: 6px 16px;
    font-size: {DesignSystem.FONTS['size_md']};
    font-weight: 500;
    min-width: 80px;
    min-height: 30px;
}}
...
QPushButton#ActionBtn {{
    background-color: {DesignSystem.COLORS['info']};
}}
QPushButton#ActionBtn:hover {{
    background-color: {DesignSystem.COLORS['info_dark']};
}}

/* 操作按钮 - 危险操作使用警示色 */
QPushButton#DangerBtn {{
    background-color: {DesignSystem.COLORS['danger']};
}}
QPushButton#DangerBtn:hover {{
    background-color: {DesignSystem.COLORS['danger_dark']};
}}
...
QLineEdit[validation="failed"] {{
    border-color: {DesignSystem.COLORS['danger']};
...
QCheckBox::indicator:checked {{
    background-color: {DesignSystem.COLORS['primary']};
    border-color: {DesignSystem.COLORS['primary_dark']};
}}
...
QLabel#TipLabel {{
    ...
    font-size: {DesignSystem.FONTS['size_sm']};
    font-style: italic;
    margin-top: 4px;
}}
"""
...
class DufsServiceDialog(QDialog):
    ...
    def _add_permission_config(self, layout: QVBoxLayout) -> None:
        ...
        perm_row_layout.setSpacing(20)

        # 允许上传
        self.upload_check: QCheckBox = QCheckBox("允许上传")
        perm_row_layout.addWidget(self.upload_check)

        # 允许删除
        self.delete_check: QCheckBox = QCheckBox("允许删除")
        perm_row_layout.addWidget(self.delete_check)

        # 允许搜索
        self.search_check: QCheckBox = QCheckBox("允许搜索")
        self.search_check.setChecked(True)
        perm_row_layout.addWidget(self.search_check)

        # 允许存档
        self.archive_check: QCheckBox = QCheckBox("允许存档")
        self.archive_check.setChecked(True)
        perm_row_layout.addWidget(self.archive_check)

        # 添加弹性空间
        perm_row_layout.addStretch()

        perm_layout.addLayout(perm_row_layout)
        
        # 允许所有权限（独立选项）
        self.allow_all_check: QCheckBox = QCheckBox("允许所有权限")
    ...
    def _on_allow_all_changed(self, state: int) -> None:
        """允许所有权限复选框状态改变事件"""
        if state == Qt.Checked:
            # 勾选"允许所有"时，自动勾选所有权限
